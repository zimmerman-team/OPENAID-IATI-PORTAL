  ...
  */
  function SearchController($scope, $state, Aggregations, TransactionAggregations, Activities, templateBaseUrl, $timeout, $location) {
    ...
    vm.search = function(){
      ...
      Activities.searchList('&q_fields=iati_identifier,title,description&q_lookup=startswith&q='+vm.searchString).then(function(data, status, headers, config){
      ...
      Aggregations.aggregation('recipient_country', 'count', '&q_fields=recipient_country&q_lookup=startswith&q=' + vm.searchString, '-count', 3).then(function(data, status, headers, config){
      ...
      Aggregations.aggregation('recipient_region', 'count', '&q_fields=recipient_region&q_lookup=startswith&q=' + vm.searchString, '-count', 3).then(function(data, status, headers, config){
      ...
      Aggregations.aggregation('sector', 'count', '&q_fields=sector&q_lookup=startswith&q=' + vm.searchString, '-count', 3).then(function(data, status, headers, config){
      ...
      TransactionAggregations.aggregation('receiver_org', 'activity_count', '&q_fields=participating_org&q_lookup=startswith&q=' + vm.searchString, '-activity_count', 3).then(function(data, status, headers, config){
