  ...
  */
  function SearchController($scope, $state, Aggregations, TransactionAggregations, Activities, templateBaseUrl, $timeout, $location) {
    ...
    vm.search = function(){
      ...
      Activities.searchList('&fields=iati_identifier,title,description&q='+vm.searchString).then(function(data, status, headers, config){
      ...
      Aggregations.aggregation('recipient_country', 'count', '&fields=recipient_country&q=' + vm.searchString, '-count', 3).then(function(data, status, headers, config){
      ...
      Aggregations.aggregation('recipient_region', 'count', '&fields=recipient_region&q=' + vm.searchString, '-count', 3).then(function(data, status, headers, config){
      ...
      Aggregations.aggregation('sector', 'count', '&fields=sector&q=' + vm.searchString, '-count', 3).then(function(data, status, headers, config){
      ...
      TransactionAggregations.aggregation('receiver_org', 'activity_count', '&fields=participating_org&q=' + vm.searchString, '-activity_count', 3).then(function(data, status, headers, config){
